...
class Discriminator(torch.nn.Module):
    def __init__(self, in_channels=1, dim=64, out_conv_channels=1):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv3d(
                in_channels=3 * conv1_channels, out_channels=conv2_channels, kernel_size=1,
                stride=1, padding=0, bias=False
            ),
    ...
    def forward(self, map1, map2, image):
        ...
        map_feature1 = self.conv11(map1)
        map_feature2 = self.conv12(map2)
        image_feature = self.conv13(image)
        x = torch.cat([map_feature1, map_feature2, image_feature],dim=1)
        x = self.conv2(x)
        x = self.conv3(x)
        x = self.conv4(x)
