...
parser.add_argument('--exp', type=str, default='LUD_test1', help='model_name')
...
def loss_fn(candidates, prototype):
    # 检查输入数据
    if torch.any(torch.isnan(candidates)):
        print("candidates_nan")
        return torch.tensor(0)
    if torch.any(torch.isnan(prototype)):
        print("prototype_nan")
        return torch.tensor(0)

    epsilon = 1e-9
    # 归一化并确保连续性
    x = F.normalize(candidates + epsilon, dim=0, p=2).permute(1, 0).unsqueeze(0).contiguous()
    y = F.normalize(prototype + epsilon, dim=0, p=2).permute(1, 0).unsqueeze(0).contiguous()
...
T = 0.1
Good_student = 0
def compute_uxi_loss(predicta, predictb, represent_a, preds, lamda, percent=20):
    ...
    with torch.no_grad():
      ...
      prob_b = predictb
      u = -1.0 * torch.sum(preds * torch.log(preds + 1e-6), dim=1)
      u_clone = u
      thresh_b = np.percentile(u_clone.detach().cpu().numpy().flatten(), percent)
      thresh_mask_b = u_clone.ge(thresh_b).bool()
      if not u_clone.is_contiguous():
          print("u_clone wrong")
      ...
      num_samples = forground_candidate.size(1) // 10000
    ...
    if not forground_candidate[:, selected_indices_f].is_contiguous():
        print("forground_candidate[:, selected_indices_f]_clone wrong")
    if not background_candidate[:, selected_indices_b].is_contiguous():
        print("background_candidate[:, selected_indices_b]_clone wrong")
    if not prototype_b.unsqueeze(dim=1).is_contiguous():
        print("prototype_b.unsqueeze(dim=1) wrong")
    if not prototype_f.unsqueeze(dim=1).is_contiguous():
        	print("prototype_f.unsqueeze(dim=1) wrong")
    ...
    con_loss = contrastive_loss_c + contrastive_loss_b + contrastive_loss_f
    if not contrastive_loss_f.is_contiguous():
        print("contrastive_loss_f wrong")
    if not contrastive_loss_b.is_contiguous():
        print("contrastive_loss_b wrong")
    if not contrastive_loss_c.is_contiguous():
        print("contrastive_loss_c wrong")
    if not con_loss.is_contiguous():
        print("con_loss wrong")
    weight = batch_size * h * w * d / torch.sum(target != 2)
    #  loss_a = (weight * F.cross_entropy(predicta, target, ignore_index=2) * lamda.mean(1)).sum() /(2*torch.sum(lamda)+1e-16)
    # loss_b = (weight * F.cross_entropy(predictb, target, ignore_index=2) * lamda.mean(1)).sum()/(2*torch.sum(lamda)+1e-16)
    loss_a = weight * F.cross_entropy(predicta, target, ignore_index=2)
    loss_b = weight * F.cross_entropy(predictb, target, ignore_index=2)

    return loss_a, loss_b, 0.5 * con_loss
...
if __name__ == "__main__":
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            uncertainty_clone = uncertainty.clone().detach()
            ...
            if not consistency_dist.is_contiguous():
                print("consistency_dist wrong")
            loss_u_v, loss_u_t, con_loss = compute_uxi_loss(v_outputs_soft[labeled_bs:, :, :, :, :], teacher_output_soft, v_rep[labeled_bs:], preds, weight_pixel_t_clone)
            consistency_loss = consistency_dist + loss_adv + con_loss
            consistency_weight = get_current_consistency_weight(iter_num // 150)
            loss = r_supervised_loss + v_supervised_loss + loss_u_v + loss_u_t + consistency_weight * consistency_loss
            if not (r_supervised_loss + v_supervised_loss).is_contiguous():
                print("r_supervised_loss + v_supervised_loss  wrong")
            if not consistency_loss.is_contiguous():
                print("consistency_dist wrong")
            if not loss_u_v.is_contiguous():
                print("loss_u_v wrong")
            if not loss_u_t.is_contiguous():
                print("loss_u_t wrong")
            ...
            try:
                ...
                print(f'Errpr during backward:{e}')
            vnet_optimizer.step()
            ...
            logging.info(
                'iteration ： %d con_loss : %f v_supervised_loss : %f v_loss_seg : %f v_loss_seg_dice : %f  r_supervised_loss : %f r_loss_seg : %f r_loss_seg_dice : %f consistency_loss: %f   loss_adv : %f loss_u_v : %f  loss_u_t : %f' %
                (iter_num, con_loss.item(),
                 v_supervised_loss.item(), v_loss_seg.item(), v_loss_seg_dice.item(),
                 r_supervised_loss.item(), r_loss_seg.item(), r_loss_seg_dice.item(),
                 consistency_loss.item(), loss_adv.item(),loss_u_v.item(), loss_u_t.item()))
            # change lr
