...
def ifcangetLargestCC(segmentation):
    labels = label(segmentation)
    if labels.max() != 0:
        return 1
    else:
        return 0
def test_all_case(args, net, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=False):
    total_metric = 0.0
    ith = 0
    for image_path in tqdm(image_list):
        ...
        if args.dataset == 'BRATS':
            nms = 0
        labels = ifcangetLargestCC(prediction)
        if labels == 0:
            nms = 0
        if nms:
