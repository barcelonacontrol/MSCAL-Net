...
parser.add_argument('--pretrain_path', type=str,default="./pred_sup/VNet_bs_4_8labels_beta_0.3/iter_8800.pth", help='Name of Experiment')
...
parser.add_argument('--exp', type=str, default='presup_VNet_bs_4_Dfirst', help='model_name')
...
if __name__ == "__main__":
    ...
    def create_model(name='vnet', ema=False, pretrain_path=False):
        # Network definition
        if name == 'vnet':
    ...
    model_resnet = create_model(name='vnet', ema=False, pretrain_path=args.pretrain_path)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss = v_supervised_loss + consistency_weight * consistency_loss + loss_u_v + loss_u_t
            ...
            logging.info(
                'iteration ： %d v_supervised_loss : %f v_loss_seg : %f v_loss_seg_dice : %f  r_loss_seg : %f r_loss_seg_dice : %f Good_student: %f' %
                (iter_num,
                 v_supervised_loss.item(), v_loss_seg.item(), v_loss_seg_dice.item(),
                  r_loss_seg.item(), r_loss_seg_dice.item(), Good_student))
            # change lr
